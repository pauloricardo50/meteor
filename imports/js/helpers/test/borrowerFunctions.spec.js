...
describe('Borrower Functions', () => {
  ...
  describe('getBonusIncome', () => {
    ...
    it('returns half of 1 bonus', () => {
      expect(getBonusIncome({ bonus: { bonus2014: 100 } })).to.equal(50);
    });

    it('returns half of average 2 bonuses', () => {
      expect(
        getBonusIncome({ bonus: { bonus2014: 100, bonus2015: 0 } }),
      ).to.equal(25);
    });
    ...
    it('returns 0 if an invalid bonus is given', () => {
      expect(getBonusIncome({ bonus: { bonus2014: 'hi' } })).to.equal(0);
    });
  ...
  describe('getBorrowerSalary', () => {
    ...
    it("returns sum of borrowers' salary", () => {
      expect(getBorrowerSalary({ salary: 1 })).to.equal(1);
      expect(getBorrowerSalary([{ salary: 1 }, { salary: 2 }])).to.equal(3);
    });
  });
});
