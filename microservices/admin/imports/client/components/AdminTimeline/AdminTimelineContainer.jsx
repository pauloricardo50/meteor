...
export default withProps(({ docId, collection, frontTagId }) => {
  ...
  const { loading: loadingActivities, data: activities } = useStaticMeteorData({
  ...
  const { loading: loadingTasks, data: completedTasks } = useStaticMeteorData(
    {
      ...
      params: {
        $filters: { ...queryFilters, status: TASK_STATUS.COMPLETED },
        completedAt: 1,
        title: 1,
      },
      refetchOnMethodCall: [
