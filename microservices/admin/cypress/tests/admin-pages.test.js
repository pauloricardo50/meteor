...
const pages = {
  ...
  admin: {
    ...
    User: ({ user: { _id } }) => route(`/users/${_id}`, { shouldRender: '.single-user-page' }),

    Loans: route('/loans', {
      shouldRender: '.all-loans-table',
    }),

    Loan: ({ step3Loan: { _id } }) => route(`/loans/${_id}`, {
      shouldRender: '.overview-tab',
    }),

    'Loan Overview Tab': ({ step3Loan: { _id } }) => route(`/loans/${_id}/overview`, {
      shouldRender: '.overview-tab',
    }),
    'Loan Borrowers Tab': ({ step3Loan: { _id } }) => route(`/loans/${_id}/borrowers`, {
      shouldRender: '.single-borrower-tab',
    }),
    'Loan Property Tab': ({ step3Loan: { _id } }) => route(`/loans/${_id}/property`, {
      shouldRender: '.single-property-page .map-with-marker',
    }),
    'Loan Offers Tab': ({ step3Loan: { _id } }) => route(`/loans/${_id}/offers`, {
      shouldRender: '.offers-tab',
    }),
    'Loan Communication Tab': ({ step3Loan: { _id } }) => route(`/loans/${_id}/communication`, {
      shouldRender: '.communication-tab',
    }),
    'Loan Analytics Tab': ({ step3Loan: { _id } }) => route(`/loans/${_id}/analytics`, {
      shouldRender: '.mixpanel-analytics',
    }),

    'Loan Forms Tab': ({ step3Loan: { _id, borrowers } }) => route(`/loans/${_id}/forms`, {
    ...
    'Loan Documents Tab': ({ step3Loan: { _id } }) => route(`/loans/${_id}/files`, {
      shouldRender: '.files-tab, .new-document-form',
    }),
    'Loan Actions Tab': ({ step3Loan: { _id } }) => route(`/loans/${_id}/actions`, {
      shouldRender: '.actions-tab',
    }),

    Property: ({ property: { _id } }) => route(`/properties/${_id}`, {
      shouldRender: '.single-property-page .map-with-marker',
    }),
    ...
    Borrower: ({ borrower: { _id } }) => route(`/borrowers/${_id}`, {
