...
export default class Linker {
  ...
  constructor(mainCollection, linkName, linkConfig) {
    ...
    this.linkConfig = { ...LinkConfigDefaults, ...linkConfig };
  ...
  createLink(object, collection = null) {
    const helperClass = this._getHelperClass();
  ...
  _validateAndClean() {
    ...
    } else if (this.linkConfig.field == this.linkName) {
      throw new Meteor.Error(
        'invalid-config',
   ...
   */
  _generateFieldName() {
    const cleanedCollectionName = this.linkConfig.collection._name.replace(
      /\./g,
      '_',
    );
    const defaultFieldPrefix = `${this.linkName}_${cleanedCollectionName}`;
  ...
  _handleReferenceRemovalForVirtualLinks() {
    this.mainCollection.after.remove((userId, doc) => {
      ...
      _.each(accessor.fetchAsArray(), linkedObj => {
        ...
        if (relatedLinker) {
          const link = relatedLinker.createLink(linkedObj);
  ...
  _initIndex() {
    if (Meteor.isServer) {
      ...
      } else if (this.linkConfig.unique) {
        if (this.isVirtual()) {
          throw new Meteor.Error(
  ...
  _initDenormalization() {
    ...
    if (this.isVirtual()) {
      const inversedLink = this.linkConfig.relatedLinker.linkConfig;

      const type = inversedLink.type == 'many' ? 'many-inverse' : 'inversed';
