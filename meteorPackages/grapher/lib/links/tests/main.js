...
//    ResolverCollection
// } from './collections.js';
const PostCollection = new Mongo.Collection('test_post');
const CategoryCollection = new Mongo.Collection('test_category');
const CommentCollection = new Mongo.Collection('test_comment');
const ResolverCollection = new Mongo.Collection('test_resolver');
...
describe('Collection Links', function() {
  ...
  it('Tests One', function() {
    const postId = PostCollection.insert({ text: 'abc' });
    const categoryId = CategoryCollection.insert({ text: 'abc' });
  ...
  it('Tests Many Meta', function() {
    ...
    const metaComment = CommentCollection.findOne(commentId);
    const metaPostLink = CommentCollection.getLink(metaComment, 'metaPost');
  ...
  it('Tests $meta filters for One & One-Virtual', function() {
    ...
    const categoryId = CategoryCollection.insert({ text: 'abc' });
    const post = PostCollection.findOne(postId);
    const postMetaCategoryLink = PostCollection.getLink(post, 'metaCategory');
  ...
  it('Tests $meta filters for Many & Many-Virtual', function() {
    const postId = PostCollection.insert({ text: 'abc' });
    const commentId1 = CommentCollection.insert({ text: 'abc' });
    const commentId2 = CommentCollection.insert({ text: 'abc' });

    const postMetaCommentsLink = PostCollection.getLink(postId, 'metaComments');
  ...
  it('Tests inversedBy findings', function() {
    ...
    let post = PostCollection.findOne(postId);
    const comment = CommentCollection.findOne(commentId);
    const commentsLink = PostCollection.getLink(post, 'comments');
    const commentsUniqueLink = PostCollection.getLink(post, 'commentsUnique');
    const metaCommentsLink = PostCollection.getLink(post, 'metaComments');
    const postLink = CommentCollection.getLink(comment, 'post');
    const postUniqueLink = CommentCollection.getLink(comment, 'postUnique');
    const postMetaLink = CommentCollection.getLink(comment, 'metaPost');
  ...
  it('Should auto-save object', function() {
    const comment = { text: 'abc' };

    const postId = PostCollection.insert({ text: 'hello' });
  ...
  it('Should have indexes set up', function() {
    ...
    const found = _.find(indexes, index => index.key.commentIds == 1);
  ...
  it('Should allow actions from inversed links', function() {
    ...
    const insertedPostViaVirtual = PostCollection.findOne({
      text: 'hi there',
    });
    ...
    const postsCategoryLink = CategoryCollection.getLink(category, 'posts');
    ...
    // TESTING META
    const categoryMetaPostLink = CategoryCollection.getLink(
      category,
      'metaPosts',
    );
    ...
    const postMetaCategoryLink = PostCollection.getLink(
  ...
  it('Should not result in duplicate key error on Many Unique links', function() {
    const postIdA = PostCollection.insert({ text: 'abc' });
    const postIdB = PostCollection.insert({ text: 'abc' });
