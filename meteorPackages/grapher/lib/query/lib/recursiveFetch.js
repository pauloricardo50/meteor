...
import prepareForDelivery, {
  assembleMetadata,
  removeLinkStorages,
  storeOneResults,
} from './prepareForDelivery';
...
function fetch(node, parentObject, fetchOptions = {}) {
  const { filters, options } = applyProps(node);
  ...
  if (fetchOptions.scoped) {
    _.extend(filters, {
      [`_query_path_${getNodeNamespace(node)}`]: { $exists: true },
    });
  }
  ...
  if (parentObject) {
    const accessor = node.linker.createLink(parentObject, node.collection);
