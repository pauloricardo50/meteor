...
export function storeOneResults(node, sameLevelResults) {
  ...
  node.collectionNodes.forEach(collectionNode => {
    ...
    if (collectionNode.isOneResult) {
      _.each(sameLevelResults, result => {
        if (
          result[collectionNode.linkName] &&
          _.isArray(result[collectionNode.linkName])
        ) {
...
function storeMetadata(element, parentElement, storage, isVirtual) {
  if (isVirtual) {
    ...
    if (_.isArray(storage)) {
      $metadata = _.find(
        storage,
        storageItem => storageItem._id == parentElement._id,
      );
    } else {
      $metadata = storage;
    }
    ...
    if (_.isArray(storage)) {
      $metadata = _.find(
        storage,
        storageItem => storageItem._id == element._id,
      );
    } else {
      $metadata = storage;
    }
