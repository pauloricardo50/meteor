...
export default (childCollectionNode, { limit, skip, metaFilters }) => {
  ...
  const { parent } = childCollectionNode;
  const { linker } = childCollectionNode;

  const { strategy } = linker;
  ...
  if (strategy === 'one') {
    parent.results.forEach(parentResult => {
      ...
      ] = filterAssembledData(resultsByKeyId[value], { limit, skip });
    });
  }
  ...
  if (strategy === 'one-meta') {
    parent.results.forEach(parentResult => {
      ...
      const { _id } = parentResult[fieldStorage];
      parentResult[
        childCollectionNode.linkName
      ] = filterAssembledData(resultsByKeyId[_id], { limit, skip });
    });
  }
