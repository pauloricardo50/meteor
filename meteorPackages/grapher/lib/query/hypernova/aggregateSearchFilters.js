...
function extractIdsFromArray(array, field) {
  return (array || [])
    .map(obj => (_.isObject(obj) ? dot.pick(field, obj) : undefined))
    .filter(v => !!v);
}
...
export default class AggregateFilters {
  ...
  createOne() {
    if (!this.isVirtual) {
      return {
        _id: {
          $in: _.uniq(
            extractIdsFromArray(this.parentObjects, this.linkStorageField),
          ),
        },
      };
    }
    ...
    if (!this.isVirtual) {
      ...
      if (this.metaFilters) {
        eligibleObjects = _.filter(this.parentObjects, object =>
          sift(this.metaFilters)(object[this.linkStorageField]),
        );
      }
  ...
  createMany() {
    if (!this.isVirtual) {
      ...
      const arrayOfIds = _.union(
        ...extractIdsFromArray(this.parentObjects, root),
      );
      return {
        _id: {
          $in: _.uniq(
            nested.length > 0
              ? extractIdsFromArray(arrayOfIds, nested.join('.'))
              : arrayOfIds,
          ),
        },
      };
