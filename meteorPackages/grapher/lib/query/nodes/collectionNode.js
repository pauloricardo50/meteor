...
export default class CollectionNode {
  constructor(collection, body = {}, linkName = null) {
    if (collection && !_.isObject(body)) {
      throw new Meteor.Error(
        'invalid-body',
        `The field "${linkName}" is a collection link, and should have its body defined as an object.`,
      );
    }
  ...
  applyFields(filters, options) {
    ...
    _.each(filters, (value, field) => {
      ...
      if (
        !_.contains(['$or', '$nor', '$not', '$and', '$meta', '$text'], field)
      ) {
  ...
  hasField(fieldName, checkNested = false) {
    ...
    const result = !!_.find(this.fieldNodes, fieldNode =>
      _.contains(options, fieldNode.name),
    );
   ...
   */
  getField(fieldName) {
    return _.find(this.fieldNodes, fieldNode => fieldNode.name == fieldName);
  }
  ...
  hasCollectionNode(name) {
    return !!_.find(this.collectionNodes, node => node.linkName == name);
  }
   ...
   */
  hasReducerNode(name) {
    return !!_.find(this.reducerNodes, node => node.name == name);
  }
...
export function runFieldSanityChecks(fieldName) {
  ...
  if (fieldName[0] === '$') {
    throw new Error(
      `You are not allowed to use fields that start with $ inside a reducer's body: ${fieldName}`,
    );
  }
