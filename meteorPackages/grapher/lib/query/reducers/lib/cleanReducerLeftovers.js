  ...
  _.each(root.collectionNodes, node => {
    ...
    if (node.isOneResult) {
      childResults = results
        .map(result => result[node.linkName])
        .filter(element => !!element);
    } else {
      childResults = _.flatten(
        results
          .map(result => result[node.linkName])
          .filter(element => !!element),
      );
...
function cleanNestedFields(parts, results, root) {
  ...
  const fieldName = snapCacheField || parts[0];
  ...
  results.forEach(result => {
    if (
      _.isObject(result[fieldName]) &&
      _.keys(result[fieldName]).length === 0
    ) {
      if (fieldName !== '_id') {
        delete result[fieldName];
      }
