...
import { createNodes, specialFields } from '../../lib/createGraph';
...
export function handleAddReducer(fieldName, { body, reduce, expand }, root) {
  if (!root.hasReducerNode(fieldName)) {
    const childReducerNode = new ReducerNode(fieldName, {
      body,
      reduce,
      expand,
    });
...
function addFieldIfRequired(root, fieldName, body, reducerNode) {
  if (!root.hasField(fieldName, true)) {
    ...
    const nestedFields = root.fieldNodes.filter(({ name }) =>
    ...
    if (!reducerNode || !reducerNode.isExpander) {
      ...
      fieldNode.scheduledForDeletion = nestedFields.every(
        field => field.scheduledForDeletion,
      );
    }
