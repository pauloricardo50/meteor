            ...
            items: [
                ...
                },
                {
                    ...
                    percent: () =>
                        borrowers.reduce(
                            (res, b) =>
                                b.logic.hasValidatedFinances ? res + 1 : res,
                            0
                        ) / borrowers.length
                },
                {
                    ...
                    percent: () =>
                        filesPercent(borrowers, borrowerFiles, 'auction'),
                    isDone() {
                        return this.percent() >= 1;
                    }
                },
    ...
    formArray.forEach(i => {
        if (shouldCountField(i)) {
            if (i.type === 'conditionalInput') {
                if (
                    getCurrentValue(i.inputs[0], doc) === i.conditionalTrueValue
                ) {
...
export const auctionFilesPercent = borrowers => {
    ...
    arrayify(borrowers).forEach(b => {
        ...
        if (isDemo()) {
            ...
            fileArray.forEach(
                f => f.condition !== false && a.push(b.files[f.id])
            );
        }
    });
                ...
                if (!(f.required === false || f.condition === false)) {
                    if (checkValidity) {
                        a.push(
                            ...
                            doc2.files[f.id].every(
                                file => file.status === FILE_STATUS.VALID
                            )
                                ? true
                                : undefined
                        );
...
export const closingPercent = loan => {
    ...
    closingSteps.forEach(step => {
        if (step.type === CLOSING_STEPS_TYPE.TODO) {
            arr.push(
                step.status === CLOSING_STEPS_STATUS.VALID ? true : undefined
            );
        } else {
            arr.push(
                isArray(loan.files[step.id]) &&
                loan.files[step.id].every(
                    file => file.status === CLOSING_STEPS_STATUS.VALID
                )
                    ? true
                    : undefined
            );
