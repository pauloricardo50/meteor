...
describe('Request functions', () => {
  ...
  describe('Get project value', () => {
    ...
    it('Should return 1.27M with 1M property, 200k property work, and 200k insuranceFortuneUsed and a primary residence', () => {
      props.loanRequest.general.insuranceFortuneUsed = 200000;
    ...
    it('Should return 1.25M with 1M property, 200k property work, and 200k insuranceFortuneUsed for a non primary residence', () => {
      props.loanRequest.general.usageType = 'INVESTMENT';
      props.loanRequest.general.insuranceFortuneUsed = 200000;
  ...
  describe('Get loan value', () => {
    ...
    it("Should return 800'000 with 1M property, 250k cash", () => {
      ...
      expect(getLoanValue({ loanRequest, property })).to.equal(800000);
    });
    ...
    it("Should return 600'000 with 1M property, 270k fortune, 200k insurance fortune", () => {
      ...
      expect(getLoanValue({ loanRequest, property })).to.equal(600000);
    });
    ...
    it("Should return 700'000 with 1M property, 260k fortune, 100k insurance fortune", () => {
      ...
      expect(getLoanValue({ loanRequest, property })).to.equal(700000);
    });
  });
  ...
  describe('getMonthlyWithOffer', () => {
    ...
    it('returns 0 if interests is wrong', () => {
      expect(getMonthlyWithOffer({ property, loanRequest, offer })).to.equal(0);
    });

    it('returns the right value if everything is wired up', () => {
      offer.standardOffer.test = 0.01;
      expect(getMonthlyWithOffer({ property, loanRequest, offer })).to.equal((5000 + 800000 * 0.0125 + 600) / 12);
    });
  });
  ...
  describe('getPropAnrWork', () => {
    ...
    it('should add value and propertyWork', () => {
      expect(getPropAndWork({
        property: { value: 1 },
        loanRequest: { general: { propertyWork: 2 } },
      })).to.equal(3);
    });
  });
  ...
  describe('getTotalUsed', () => {
    it('should add fortuneUsed and insuranceFortuneUsed', () => {
      expect(getTotalUsed({ loanRequest: { general: { fortuneUsed: 1 } } })).to.equal(1);
      expect(getTotalUsed({
        loanRequest: { general: { fortuneUsed: 1, insuranceFortuneUsed: 2 } },
      })).to.equal(3);
      expect(getTotalUsed({
        loanRequest: {
  ...
  describe('disableForms', () => {
    it('returns true when the right conditions are true', () => {
      expect(disableForms({ loanRequest: { logic: { step: 2 } } })).to.equal(true);
      expect(disableForms({
        loanRequest: { logic: { verification: { requested: true } } },
      })).to.equal(true);
      expect(disableForms({
        loanRequest: { logic: { verification: { validated: true } } },
      })).to.equal(true);
    });

    it("returns false if the conditions aren't met", () => {
      expect(disableForms({ loanRequest: { logic: { step: 1 } } })).to.equal(false);
      expect(disableForms({ loanRequest: { logic: { step: 0 } } })).to.equal(false);
      expect(disableForms({ loanRequest: { verification: { requested: false } } })).to.equal(false);
      expect(disableForms({ loanRequest: { verification: { validated: false } } })).to.equal(false);
    });
  });
  ...
  describe('isRequestValid', () => {
    ...
    it('should throw for insufficient revenues', () => {
      expect(() =>
        isRequestValid({ loanRequest, borrowers, property })).to.throw('income');
    });

    it('should throw for insufficient cash', () => {
      // 150k is required here
      loanRequest.general.fortuneUsed = 140000;
      loanRequest.general.insuranceFortuneUsed = 250000;
      borrowers = [{ salary: 300000 }];
      expect(() =>
        isRequestValid({ loanRequest, borrowers, property })).to.throw('cash');
    });

    it('should throw for insufficient own funds', () => {
      loanRequest.general.fortuneUsed = 160000;
      loanRequest.general.insuranceFortuneUsed = 80000;
      borrowers = [{ salary: 300000 }];
      expect(() =>
        isRequestValid({ loanRequest, borrowers, property })).to.throw('ownFunds');
    });
  });
