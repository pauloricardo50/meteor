...
describe('intl-server', () => {
  ...
  it('formats additional values property', () => {
    expect(Intl.formatMessage({ id: 'BorrowerHeader.title', values: { index: 10 } })).to.equal('Emprunteur 10');
  });
  ...
  it('uses the custom fallback if it is not undefined', () => {
    expect(Intl.formatMessage({ id: 'fakeId', fallback: 'custom' })).to.equal('custom');
