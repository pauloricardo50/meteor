...
describe('REST: setPropertyUserPermissions', function () {
  ...
  it('throws if user cannot modify property', () => {
    const body = {
      email: 'pro@org.com',
      permissions: {
        canModifyProperty: false,
    ...
    return setPropertyUserPermissions({
      ...
      expect(message).to.contain('Vous ne pouvez pas modifier');
    });
  });
  ...
  it('throws when impersonating a user from a different organisation', () => {
    const body = {
      email: 'pro2@org.com',
      permissions: {
        canModifyProperty: true,
    ...
    return setPropertyUserPermissions({
      ...
      propertyId: 'prop',
      body,
      impersonateUser: 'pro4@org2.com',
    }).then((response) => {
      const { message } = response;
      expect(message).to.contain('User with email address "pro4@org2.com" is not part of your organisation');
    });
  });
});
